...
exports.click = function(selector, done) {
  ...
  debug('.wait() for '+selector+' element');
  waitelem.apply({ timeout: null }, [this, selector, thenClick]);
};
...
exports.type = function() {
  ...
  var self = this;
  function thenType() {
    focusSelector.bind(self)(function(err) {
      if(err) {
        debug('Unable to .type() into non-existent selector %s', selector);
        return done(err);
      }

      var blurDone = blurSelector.bind(self, done, selector);
      if ((text || '') == '') {
        self.evaluate_now(function(selector) {
  ...
  waitelem.apply({ timeout: null }, [this, selector, thenType]);
...
function waitfn() {
  ...
  var timeoutTimer = setTimeout(function(){
    ...
    var strFn = args[1].toString();
    var fnArgs = sliced(args, 2, args.length-1)
    var message = `.wait() timed out after ${self.options.waitTimeout}msec, trying to execute:\n${strFn}`
    if (fnArgs.length > 0) { message += ` with args ${fnArgs}` }
